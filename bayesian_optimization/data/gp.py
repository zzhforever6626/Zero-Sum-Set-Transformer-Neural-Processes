...
__all__ = ["GPPriorSampler", 'GPSampler', 'RBFKernel', 'PeriodicKernel', 'Matern52Kernel', "RQKernel"]
...
class RQKernel(object):
    """
    Rational Quadratic kernel:
    k(x,x') = s^2 * (1 + ||x-x'||^2 / (2 * alpha * l^2))^{-alpha}
    """
    def __init__(self, sigma_eps=2e-2, max_length=0.6, max_scale=1.0, max_alpha=2.0):
        self.sigma_eps = sigma_eps
        self.max_length = max_length
        self.max_scale = max_scale
        self.max_alpha = max_alpha

    # x: batch_size * num_points * dim  [B,N,D]
    def __call__(self, x):
        # sample per-batch hyperparameters (same style as your existing kernels)
        length = 0.1 + (self.max_length - 0.1) * torch.rand(
            [x.shape[0], 1, 1, 1], device=x.device
        )  # [B,1,1,1]
        scale = 0.1 + (self.max_scale - 0.1) * torch.rand(
            [x.shape[0], 1, 1], device=x.device
        )  # [B,1,1]
        alpha = 0.1 + (self.max_alpha - 0.1) * torch.rand(
            [x.shape[0], 1, 1], device=x.device
        )  # [B,1,1]

        # normalized squared distance: [B,N,N]
        dist2 = ((x.unsqueeze(-2) - x.unsqueeze(-3)) / length).pow(2).sum(-1)

        cov = scale.pow(2) * (1.0 + dist2 / (2.0 * alpha)).pow(-alpha) \
