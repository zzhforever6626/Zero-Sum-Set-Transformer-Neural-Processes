...
def main():
    ...
    parser.add_argument('--bo_mode', choices=['oracle', 'models', 'plot'], default='models')
...
def plot(args):
    all_kernels = ['rbf', 'matern']
    kernel_names = ['RBF', 'Matern']
    all_models = ["np", "tnpd", "lbanp", "cnp", "tnp0"]
    model_names = ["NP", "TNP", "LBANP", "CNP", "ZSSTNP"]
    colors = ['navy', 'red', "deepskyblue", 'darkred', 'orange']

    n_kernels = len(all_kernels)

    fig_width = 6 * n_kernels
    fig, axes = plt.subplots(1, n_kernels, figsize=(fig_width, 8), squeeze=False)
    axes = axes[0]
    ...
    save_path = osp.join(results_path, 'gp_plot.png')
