...
from bayeso_benchmarks.two_dim_schaffer import SchafferN2
from bayeso_benchmarks.two_dim_griewank import Griewank
...
def main():
    ...
    parser.add_argument('--model', default='zstnp')
...
def plot(
    ...
    model_names = ['NP', 'TNP', 'LBANP', 'CNP', 'ZSSTNP']
    table = {2: {'function': ['dropwave','michalewicz'], 'min': 30, 'max': 128, 'step': 100000}}
    ...
    for i, dim in enumerate([2]):
        ...
        if fig is None:
            ...
            fig, axes = plt.subplots(1, n_funcs, figsize=(6 * n_funcs, 8), squeeze=False)
        axes_arr = axes[0]
        ...
        for k, m in enumerate(color.keys()):
            for j, function in enumerate(functions):
                ...
                ax = axes_arr[j]
                ax.plot(np.arange(num_iter + 1), mean, label=model_names[k], lw=2.1, color=color[m])
                ax.fill_between(np.arange(num_iter + 1), mean - std, mean + std, alpha=0.1, color=color[m])
                ax.set_title(f'{dim}D {function}', fontsize=20)
    ...
    fig.legend(by_label.values(), by_label.keys(),
               loc="lower center", fancybox=True, shadow=True,
               ncol=min(11, len(by_label)), fontsize=16, facecolor='white')
