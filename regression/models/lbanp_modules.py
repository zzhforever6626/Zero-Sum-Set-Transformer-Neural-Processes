...
class LBANPEncoderLayer(nn.Module):
    def __init__(self,
        ...
        if norm_first:
            self.latent_self_attn = PreNorm(self.latent_dim,
                                            Attention(self.latent_dim, heads=nhead, dim_head=self.latent_dim // nhead,
                                                      dropout=dropout))
            self.latent_ff = PreNorm(self.latent_dim,
                                     FeedForward(self.latent_dim, dim_feedforward=dim_feedforward, dropout=dropout))
            # Self Attention performs the linear operations
            self.cross_attn = PreNorm(self.latent_dim, Attention(self.latent_dim, context_dim=self.d_model, heads=nhead,
                                                                 dim_head=self.latent_dim // nhead, dropout=dropout),
                                      context_dim=self.d_model)
            self.cross_ff = PreNorm(self.latent_dim,
                                    FeedForward(self.latent_dim, dim_feedforward=dim_feedforward, dropout=dropout))
        else:
            self.latent_self_attn = PostNorm(self.latent_dim,
                                             Attention(self.latent_dim, heads=nhead, dim_head=self.latent_dim // nhead,
                                                       dropout=dropout))
            self.latent_ff = PostNorm(self.latent_dim,
                                      FeedForward(self.latent_dim, dim_feedforward=dim_feedforward, dropout=dropout))
            # Self Attention performs the linear operations
            self.cross_attn = PostNorm(self.latent_dim,
                                       Attention(self.latent_dim, context_dim=self.d_model, heads=nhead,
                                                 dim_head=self.latent_dim // nhead, dropout=dropout),
                                       context_dim=self.d_model)
            self.cross_ff = PostNorm(self.latent_dim,
                                     FeedForward(self.latent_dim, dim_feedforward=dim_feedforward, dropout=dropout))
...
class NPDecoderLayer(nn.Module):
    def __init__(self,
        ...
        if norm_first:
            ...
            self.cross_attn = PostNorm(self.latent_dim, Attention(self.latent_dim, self.d_model, heads=nhead,
                                                                  dim_head=self.latent_dim // nhead, dropout=dropout),
                                       context_dim=self.latent_dim)
            self.cross_ff = PostNorm(self.latent_dim,
                                     FeedForward(self.latent_dim, dim_feedforward=dim_feedforward, dropout=dropout))
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if norm_first:
            self.dataset_self_attn = PreNorm(self.d_model,
                                             Attention(self.d_model, heads=nhead, dim_head=self.d_model // nhead,
                                                       dropout=dropout))
            self.ff = PreNorm(self.d_model, FeedForward(self.d_model, dim_feedforward=dim_feedforward, dropout=dropout))
        else:
            self.dataset_self_attn = PostNorm(self.d_model,
                                              Attention(self.d_model, heads=nhead, dim_head=self.d_model // nhead,
                                                        dropout=dropout))
            self.ff = PostNorm(self.d_model,
                               FeedForward(self.d_model, dim_feedforward=dim_feedforward, dropout=dropout))
